...
if __name__ == "__main__":
    ...
    # Select proper chain
    def route_select(info):
        if "python" in info["topic"].lower():
            return python_chain
        elif "pytorch" in info["topic"].lower():
            return pytorch_chain
        elif "scikit-learn" in info["topic"].lower():
            return scikit_chain
        else:
            return scikit_chain

    # Create a final chain
    final_chain = {"topic": topic_chain, "question": lambda x: x["question"]} | RunnableLambda(
        route_select
    )
